...
\usepackage{booktabs}
%% XeTeX supports protrusion only; font expansion is unavailable here, so short
%% paragraph-final lines are handled per-paragraph with \looseness where needed.
...
% Author list. Kekatos first, Katsaros last (per author agreement); the rest
% alphabetical by surname. NB this differs from the order registered on
% HotCRP #53 -- update HotCRP to match before upload.
...
\renewcommand{\shortauthors}{Kekatos et al.}

\keywords{runtime verification, LLM agents, agent guardrails, metric first-order
          temporal logic, provenance integrity, benchmark instrumentation, tool-call
          traces, CPS/IoT security}
...
We investigate metric first-order temporal logic (MFOTL) as a declarative substrate
for such policies, replaying the recorded trajectories that AgentDojo, STAC and R-Judge
already ship through the unmodified MonPoly engine. The STAC verdict is agreed by four
monitors: our reference implementation, MonPoly, its Isabelle/HOL-verified kernel
VeriMon, and the independently developed DejaVu, whose decision procedure differs.
...
and external-recipient obligations reduce benign firing while preserving
...
attack. Metric policies also flip verdict on identical actions purely by elapsed time.
...
%% acmart sets \flushbottom after \maketitle; the forced equal column
%% heights produce small overfull vboxes around the appendix displays.
\raggedbottom
...
semantics and a reusable, independently-maintained engine, not bespoke
...
\looseness=-1 evidence: does formal RV actually catch \emph{real} agent attacks, and what does it
...
context \emph{is} available, MFOTL expresses more discriminating policies, and
...
\begin{itemize}
\item \textbf{A declarative RV formulation and same-benchmark evaluation methodology
      for tool-using agents.} We map recorded agent trajectories into MFOTL event
      streams and evaluate reusable generic, parametric and metric obligations using
      independently implemented runtime monitors, scored against each benchmark's own
      labels (\S\ref{sec:oblig}, \S\ref{sec:method}).
\item \textbf{An empirical characterisation of when formal RV adds value.} Generic
      obligations achieve substantial attack coverage but largely collapse to
      typed-action detection on existing corpora. Where relational or timing context is
      available, data-parametric and metric MFOTL policies produce
      history-dependent verdicts and improve discrimination over action-only checking
      (\S\ref{sec:results}--\S\ref{sec:gap}).
\item \textbf{The assurance conditions for history-dependent enforcement.} We show that
      trace provenance is itself an attack surface, demonstrate a lookup-bound
      provenance mechanism that eliminates the tested poisoning attack, and use a field
      ablation and a three-benchmark readiness audit to identify the information robust
      history-dependent enforcement requires. This motivates a twelve-field
      enforcement-ready trace schema (\S\ref{sec:canon}--\S\ref{sec:gap}).
\end{itemize}
...
(agent adaptation after enforcement) and is outside the observational scope of this
work.
...
MonPoly's formally verified kernel; both support the untimed first-order past-time
fragment used in our STAC cross-validation (\S\ref{sec:results}).
...
families in edge-IoT settings~\cite{hybridhier2025,hiermon2026,critisfabric2026}, where
the compute budget a monitor must share with the application is itself a measured
constraint~\cite{v2xedge2026};
companion work of ours applies mission-level runtime assurance to LLM-assisted swarms
over a verification-aware fabric~\cite{mesasswarm2026}, where the traces are generated in
simulation, not recorded: the complementary setting to the offline replay
studied here; a
shield~\cite{shielding2018} gates the pending call on a violation.

% ============================================================================
\section{Obligations and Offline Methodology}
\label{sec:oblig}

This section fixes what we monitor and how we measure it. We first state the obligations
as MFOTL formulas over a tool-call trace, then describe the pipeline that turns a
recorded trajectory into an event stream and the measures reported throughout.

\subsection{Five generic obligations in MFOTL}
...
the trace,'' chosen by action \emph{semantics}, not by any benchmark's tool
...
% ============================================================================
\subsection{Replay pipeline and measures}
...
\end{tikzpicture}}
\looseness=-1 \caption{Offline replay pipeline: recorded trajectories are canonicalised to a typed
event stream and checked against MFOTL obligations by MonPoly; verdicts are scored
against each benchmark's own labels. No LLM or agent runs.}
\label{fig:pipeline}
\end{figure}
\label{sec:method}
...
require. VeriMon, MonPoly's formally verified kernel, covers the same
...
has interval $[22.0,37.8]$). The keyword patterns are the ordinary English verbs for
...
understated the method; they did not inflate it. The same read-only guard then had to
be applied to \emph{action typing} as well, because the financial pattern
\texttt{transaction} was matching the read-only \texttt{get\_most\_recent\_transactions}
and \texttt{get\_scheduled\_transactions}: retrieving records is not a value transfer,
and the hand-labelled gold set of \S\ref{sec:canon} marks all twenty read-only accessors
\textsf{NONE}. That second correction moved detection to $70.1\%$ and benign firing to
$29.3\%$, and those are the figures reported throughout; it also raised typing precision
(micro $0.84\!\to\!0.90$) and made the payee ablation discriminate at all, since the
read-only matches had previously kept the refined financial slot permanently lit. STAC
and R-Judge are unaffected: no tool name in either corpus collides. We report the correction instead of silently re-freezing, because it is
...
We report the generic obligations first, then the parametric ones. The contrast between
the two matters: the generic layer establishes how much of the recorded attack
surface typed-action checking reaches at all, and the parametric layer is where a verdict
starts to depend on the trace's history.

\subsection{Generic obligations on recorded trajectories}

\paragraph{AgentDojo (same-benchmark, offline)} Table~\ref{tab:agentdojo} replays
\looseness=-1 all recorded GPT-4o runs (no defence). AgentDojo's own no-defence attack-success is
$28.9\%$ ($1931/6680$). Our RV, applied to the identical runs, fires on $70.1\%$
($95\%$ CI $[68.0,72.1]$) of the \emph{successful} attacks: i.e.\ a shield would
have had a flagged call to gate in just over seven in ten of the attacks that actually got
...
Two caveats belong with these numbers. First, they are \emph{typed-action} detection:
with no approval events in the corpora the approval condition is never satisfied, so in
the monitored violation form the negated condition holds throughout and the verdict
reduces to the tool's action class; a keyword rule set would reach the same verdicts. Why that is a property of the corpora and not of MFOTL is the subject of
\S\ref{sec:gap}; the claims that do rest on the formalism are the parametric obligations
of \S\ref{sec:param} and the timed policies of \S\ref{sec:cps}. Second, \emph{a $29.3\%$
BFR is not a deployable operating point}: BFR counts runs on which some obligation fires,
a firing proxy and not a measured utility cost, since we never re-execute the blocked task.
At this rate the generic layer is a triage filter for human review, not an enforcement
gate.

\begin{table}[t]
\centering
\caption{AgentDojo recorded runs (GPT-4o, no defence) replayed offline. Detection is on
AgentDojo's \textsf{security}=true attacks, BFR on its benign runs; $96$ of $6899$ runs
with an empty tool-call chain are excluded. Per-obligation counts overlap and sum to
$1677$ against $1354$ detected, so the union is the detection figure
(Appendix~\ref{app:extended}, Table~\ref{tab:peroblig}).}
\label{tab:agentdojo}
...
attacks spans $48.4$--$91.7\%$ and BFR $16.7$--$34.8\%$. AgentDojo's own built-in
...
final harmful step, so the signal concentrates there: $57/483$ ($11.8\%$) preventive,
$297$ ($61.5\%$) last-moment, $129$ ($26.7\%$) no firing. The earliest-warning margin,
...
\paragraph{STAC (recorded chains, MonPoly)} Replaying all
...
% ============================================================================
\subsection{Parametric obligations: relational conditions over trace data}
\label{sec:param}
...
by a reusable, independently-maintained engine, and hence auditable, in place of
...
\looseness=-1 (generic O3), the parametric obligation fires only on a transfer to a payee never
seen as a \emph{structured} recipient or sender in prior account data,
\[
...
\begin{table}[t]
\centering
\caption{Parametric vs.\ generic obligations (GPT-4o), Wilson $95\%$ intervals. Each row
is a \emph{single} obligation on a \emph{single} suite, so these rates are not comparable
with the policy-set unions reported elsewhere; benign denominators are $25$ banking and
$46$ workspace runs. Appendix~\ref{app:extended} gives the conventions in full.}
\label{tab:param}
...
\paragraph{What depends on temporal reasoning} The generic rates are typed-action
detection, as \S\ref{sec:results} states. The two parametric rows of
Table~\ref{tab:param} and every verdict flip in the timed study do depend on genuine
relational or temporal reasoning: the \emph{same} call is allowed or flagged according to
a prior-provenance or prior-state condition, which a stateless per-call predicate cannot
decide and which MFOTL states declaratively instead of in bespoke stateful code.

% ============================================================================
\section{Threats to Verdict Validity}
\label{sec:canon}

A verdict is only meaningful if the two things it rests on can be trusted: the typing
that decides an action's class, and the trace content from which provenance is derived.
This section attacks both. The first is an accuracy question we measure against a
hand-labelled gold set; the second turns out to be an adversarial question, because the
provenance a policy reads is content an attacker can write.

\subsection{Canonicalisation accuracy and sensitivity}

Detection is only as good as the tool$\to$class typing, so we measure it and bound its
influence. Against a gold set of $54$ tool names hand-labelled blind to any detection
outcome, the keyword typing reaches micro-averaged precision $0.90$, recall $0.84$ and
single-primary exact-match $87.0\%$ ($47/54$), with systematic rather than random
residual errors. Re-running the replays under four tool$\to$class mappings moves AgentDojo
detection within $[69.0,79.9]\%$ and BFR within $[26.0,43.9]\%$, and STAC detection within
$[61.7,73.1]\%$: moderately sensitive, and the frozen typing is \emph{conservative} on
AgentDojo, since correcting the mapping raises detection rather than lowering it. The gold
set, the per-class table and the four mappings are in
Appendix~\ref{app:extended} (Tables~\ref{tab:canon} and~\ref{tab:canonsens}).

% ============================================================================
\subsection{Provenance integrity: the monitor's trace history is an attack surface}
\label{sec:prov}
...
\looseness=-1 Conditions (ii)--(iv) need fields the corpora do not carry, so they remain design
...
issued, while a planted message answers nothing outstanding:
\[
...
trade-off; the naive predicate was reading a field it had no warrant to trust. We
...
Provenance integrity is therefore a requirement on the trace, not a detail of
canonicalisation.
...
necessarily collapse into simple action typing.

\subsection{The readiness scorecard}

A readiness scorecard over the three
benchmarks (Table~\ref{tab:score}) makes this concrete. Explicit \emph{approvals} are almost absent: $0/6899$ ($0\%$)
AgentDojo runs, $6/483$ ($1.2\%$) STAC chains, $2/571$ ($0.4\%$) R-Judge records.
...
The metric and stateful policies that motivate temporal RV (\S\ref{sec:cps}, below) thus
...
schema, enumerated below, giving the minimum
...
approvals by \emph{tool name} and may be looking in the wrong place. We therefore
...
approval language does appear: mining the user text instead of tool names raises the
count almost twelvefold, from $6/483$ ($1.2\%$) to $70/483$ ($14.5\%$) on a loose keyword match
...
\paragraph{A control case: $\tau$-bench} If approvals were simply hard to record, no
corpus would have them. We therefore ran the same census over the $1{,}980$ recorded
trajectories that $\tau$-bench~\cite{taubench2024} ships, a \emph{task} benchmark whose
system prompt is the domain policy and whose policy requires the agent to obtain user
confirmation before mutating state. The contrast is stark. $\tau$-bench averages $7.08$
user turns per trajectory against AgentDojo's $1.00$ and STAC's $2.61$; $88.7\%$ of its
trajectories contain a mutating call; and while a tool-name search again finds
\emph{nothing} ($0$ of its $26$ tools is an approval tool, exactly as in AgentDojo),
$1039/1980 = 52.5\%$ of trajectories contain explicit permission-granting language in a
user turn positioned \emph{before} the mutating call. Every one of those $1039$ answers a
preceding assistant turn that made no tool call, i.e.\ they are confirmations of a
proposal rather than instructions. Against STAC's $0.6\%$ that is a factor of eighty. The effect is not a
single-domain quirk: the rate is $58.0\%$ and $46.5\%$ on the airline domain
($200$ and $400$ trajectories, GPT-4o and Sonnet-3.5) and $65.4\%$ and $47.4\%$ on
retail ($460$ and $920$), so it holds across both domains and both models. The airline
domain is also the travel-booking setting Maris evaluates with $20$ synthetic queries,
which is the comparison we can make: same task domain, $600$ recorded trajectories
instead of authored ones.

Two conclusions follow. The approval shortfall is a property of \emph{attack} corpora
rather than of agent traces in general, so it is a fixable instrumentation gap and not an
intrinsic limit; and the fix is already partly available, since on a corpus like
$\tau$-bench the antecedent of O1--O5 would be genuinely satisfiable and the generic
obligations would not collapse to action typing. We do not report detection on
$\tau$-bench because it carries no attack labels: it measures task success, not attack
success, which is why the detection results here use AgentDojo and STAC. The two are
complementary, and a corpus with $\tau$-bench's interaction protocol and AgentDojo's
attack labels is, on this evidence, the single most useful thing the community could
build.

\paragraph{The contribution of each field} A scorecard says which fields are missing; it does not
...
approval- and timing-dependent levels are unevaluable: \emph{zero}
...
\paragraph{Timed policies} Metric obligations are what the schema of this section would
make evaluable, and they are the one place the paper exercises MFOTL's metric operators.
Because the corpora carry no timestamps we exercise them on authored logs: three timed
policies, a multi-instance building trace and the R-Judge IoT subset, all run verbatim by
MonPoly, with a verdict that flips on identical actions purely by elapsed time. The
detail is in Appendix~\ref{app:timed}, since it demonstrates expressiveness rather than
field performance.

\section{Discussion and Conclusion}
\label{sec:concl}

\paragraph{Positioning} MFOTL and MonPoly have already been applied to agent policy
enforcement: Maris~\cite{maris2026} expresses privacy policies in MFOTL and calls MonPoly
online from inside the agent runtime, on three authored applications driven by $20$
synthetic queries each. Our question is different, and the difference is the corpus
rather than the formalism: what is measurable \emph{offline} on the attack corpora the
community ships, and what those corpora cannot support. Those applications ship no
recorded trajectories to replay, which is why we report on $6{,}899$ AgentDojo runs,
$483$ STAC chains, $571$ R-Judge records and $1{,}980$ $\tau$-bench trajectories. What
formal RV adds in that setting is the combination: trace-level scope, formal temporal
semantics, data-parametric provenance and offline evaluability in \emph{one} declarative
substrate. Table~\ref{tab:matrix} (appendix) records what the cited systems were shown to
do in their evaluated configurations, not impossibility: AgentSpec~\cite{agentspec2026}
has a formal rule semantics and some history-aware rules, so a determined encoding could
reach further than its cells show.
...
STAC lacks preconditions ($6/483$ chains by tool name; \S\ref{sec:gap}), so its generic obligations reduce to
...
\paragraph{Conclusion} Offline on real agent-attack corpora, MonPoly flags $71.8\%$ of
STAC chains and detects $70.1\%$ of GPT-4o's successful AgentDojo attacks at $29.3\%$
BFR: numbers that mostly reflect risky-action \emph{typing}, since these corpora rarely
contain the preconditions temporal logic is for. Where data-parametric and timed
reasoning applies, MFOTL states conditions a stateless per-call predicate cannot decide.
Four completed results anchor the security case: an earliest-warning margin (preventive
on $52.4\%$ of successful attacks); a provenance predicate that is itself an attack
surface, flipped $94$--$99\%$ by one planted line and closed completely, on the tested
attack family, by binding provenance to the issuing lookup; a field ablation showing the
sound and unsound variants are indistinguishable except under attack; and a readiness
audit. That last is the one we would most want carried forward, because it explains the
first: widely used agent-security corpora support attack-level evaluation but omit the
event semantics history-dependent enforcement needs. It asks two things of a future
benchmark, not one: the schema of \S\ref{sec:gap}, and an interaction protocol in which
approval can occur at all, which the $\tau$-bench control shows is the binding
constraint.
...
permission, verify, grant, two\_factor, 2fa, otp, request\_approval, acknowledge,
sign\_off, double\_check. No tool whose name begins \texttt{get\_}, \texttt{list\_},
\texttt{read\_}, \texttt{search\_}, \texttt{view\_}, \texttt{fetch\_} or
\texttt{query\_} counts as an approval event or as a typed action, since a read-only
accessor is neither a grant of permission nor a commit (\S\ref{sec:method}). This is
the corrected set used for every number reported here.
...
keep the actual times, not a dense index. Per-obligation counts are
...
\texttt{/usr/local/bin/monpoly} inside Docker image \texttt{infsec/monpoly}
(image id \texttt{sha256:82cde1a2\allowbreak3cd3}). It is invoked as
...
\url{https://github.com/nikos-kekatos/formal-rv-tool-using-llm-agents}), so the full
pipeline is
...
% ============================================================================
\section{Timed CPS/IoT Policies}
\label{sec:cps}

This is a proof of \emph{applicability} of MFOTL/MonPoly to CPS/IoT policies,
not a broad security validation on CPS/IoT deployments. Agentic components are already being proposed for such settings, together with tamper-evident evidence trails~\cite{resilagent2026}, which is what makes the trace-integrity question of \S\ref{sec:canon} concrete rather than hypothetical. We give three evidence points,
...
\paragraph{Multi-instance building actuation} On an authored $20$-event smart-building
trace ($13$ policy-relevant actions, three past-time policies, formulas in
Appendix~\ref{app:repro}) MonPoly flags exactly the $6$ history-violating instances and
clears all $7$ compliant ones. One violation is purely \emph{timed}: an enabling
\texttt{damper\_open} fell $4$\,h before a $3$\,h window, so the same compressor start is
a violation only by timing.
...
runs or records exposing each field.}
\label{tab:score}
\renewcommand{\arraystretch}{1.1}
\setlength{\tabcolsep}{5pt}
\small
...
\paragraph{Reporting conventions} Two conventions explain every apparent mismatch
between tables. \emph{Policy set}: a union over the five generic obligations is not
comparable with a single obligation on the same runs, e.g.\ workspace benign firing is
$10/46$ as a union against $7/46$ for O4 alone. \emph{Run validity}: the aggregate
replay counts only runs with a non-empty tool-call chain ($96$ banking runs excluded, one
benign), while the parametric scripts count every run with a non-empty message list,
which is why banking benign reads $19/24$ in one place and $19/25$ in another with the
same numerator.

\begin{table}[t]
\looseness=-1 \centering
...
identical under both predicates, so the hardening costs nothing measurable.}
\label{tab:harden}
...
R2 $+$ provenance   & poison-resistant & $70.0\%$ & $24.0\%$ & $\mathbf{0\%}$ \\
\midrule
R3 $+$ approval id & approval-bound  & \multicolumn{3}{c}{unevaluable ($0/1545$ runs)} \\
R4 $+$ time, state & metric          & \multicolumn{3}{c}{unevaluable ($0$ timestamps)} \\
\bottomrule
\end{tabular}
\end{table}
...
ratio $\mathrm{LR^+}$ (detection $/$ BFR). Destructive is the single most discriminating
obligation ($\mathrm{LR^+}=3.91$), ahead of export ($2.64$) and teardown never fires for GPT-4o (these suites
...
from both because it is a single obligation, not the union.}
\label{tab:suite}
\renewcommand{\arraystretch}{1.0}
...
generic detection ranges $48.4$--$91.7\%$ and BFR $16.7$--$34.8\%$.
...
conservative); the conservative mapping lowers AgentDojo detection to $69.0\%$ and BFR to $26.0\%$, and STAC further.
...
log to $0.43$--$0.45$M at $\times10$--$\times50$ and $0.8$M on the AgentDojo export,
...
paired-benign, the inconclusive per-model result), \texttt{rjudge\_rv.py},
\texttt{harden\_provenance.py} (lookup-bound provenance vs.\ the same poisoning),
\texttt{field\_ablation.py} (the R0--R4 levels),
\texttt{approval\_census.py} (approval density across AgentDojo, STAC and
$\tau$-bench), \texttt{stac\_to\_dejavu.py} (the DejaVu cross-check),
\texttt{run\_corpus.py}, \texttt{gen\_stac\_scale.py} and
\texttt{agentdojo\_to\_monpoly.py}, and the
MonPoly \texttt{.sig}/\texttt{.mfotl}/\texttt{.log} files for the STAC, building, IoT
and timed studies. Nothing beyond this list is shipped: the repository contains exactly
the scripts and specifications behind the numbers reported here.
