...
AgentDojo attacks the generic obligations detect $61.6\%$ at a $31.7\%$ benign firing
...
rate substantially at a small detection cost, both statistically significant under
exact McNemar tests. We further show an earliest-warning margin, that the provenance
...
\item Results on three corpora with Wilson $95\%$ intervals: $71.8\%$ of STAC chains
      flagged (MonPoly-confirmed); $61.6\%$ of GPT-4o's \emph{successful} AgentDojo
      attacks detected at a $31.7\%$ benign firing rate (BFR); the full R-Judge
...
small, and we report Wilson intervals on every rate (e.g.\ the $31.7\%$ aggregate BFR
...
$28.9\%$ ($1931/6680$). Our RV, applied to the identical runs, fires on $61.6\%$
($95\%$ CI $[59.4,63.8]$) of the \emph{successful} attacks --- i.e.\ a shield would
...
report it plainly: the same generic obligations fire on $31.7\%$ ($39/123$, Wilson
$95\%$ $[24.1,40.4]$) of \emph{benign} task runs, because a legitimate task also sends
...
approval antecedent of O1--O5 is vacuously unsatisfied, and a firing therefore depends
only on the tool's class. At this layer MFOTL is buying declarative uniformity, not
...
of \S\ref{sec:gap}. Second, \emph{a $31.7\%$ BFR is not a deployable operating point}.
...
\emph{overlap} --- one run can trip several obligations --- so they sum to $1524$, more
than the $1190$ runs detected; the union, not the sum, is the detection figure. Runs
...
RV detection on \emph{successful} attacks       & $1190/1931 = 61.6\%$ \\
RV fires on any attacked run                     & $2426/6680 = 36.3\%$ \\
RV BFR on benign runs                     & $39/123 = 31.7\%$ \\
\quad Wilson $95\%$ interval on BFR             & $[24.1,40.4]$ \\
\midrule
\multicolumn{2}{l}{\emph{by obligation, on the $1190$ detected (overlapping)}} \\
\quad destructive without confirmation         & $144$ \\
\quad teardown without approval                & $0$ \\
\quad financial without confirmation           & $540$ \\
\quad export without consent                   & $527$ \\
\quad actuation without precondition           & $313$ \\
\quad sum of the five (overlap $>$ union)       & $1524$ \\
\bottomrule
\end{tabular}
\end{table}
...
final harmful call (\emph{preventive}) in $991$ ($51.3\%$), fires only \emph{at} that
call (\emph{last-moment}) in $199$ ($10.3\%$), and never fires in $741$ ($38.4\%$);
where it is preventive the median detection lead is two tool-calls (mean $2.41$, max
...
obligations flag $97.7\%$ of attacks \emph{and} $95.8\%$ of benign runs --- near-total
...
obligations ($\mathrm{LR^+}=1.94$) to the parametric spec raises it on both suites ---
...
$0.84$ and single-primary exact-match $83.3\%$ ($45/54$); the residual errors are
...
detection moves in $[61.6,71.2]\%$ and BFR in $[31.7,45.5]\%$, STAC detection stays in
...
enforcement) outside our scope. Generic specs have a high BFR ($31.7\%$), and the
...
detects $61.6\%$ of GPT-4o's successful AgentDojo attacks at $31.7\%$ BFR --- numbers
...
(preventive on $51.3\%$ of successful AgentDojo attacks, median lead two tool-calls);
...
=(1931-1190)/6680=11.1\%$; this is a descriptive count of successful attacks the
...
\midrule
O1 destructive & $7.5$  & $3.3$  & $105$ & $2.29$ \\
O2 teardown    & $0$    & $0$    & $0$   & --     \\
O3 financial   & $28.0$ & $17.1$ & $245$ & $1.64$ \\
O4 export      & $27.3$ & $10.6$ & $488$ & $2.58$ \\
O5 actuation   & $16.2$ & $6.5$  & $18$  & $2.49$ \\
\midrule
union (any)    & $61.6$ & $31.7$ & --    & $1.94$ \\
\bottomrule
\end{tabular}
\end{table}
...
banking BFR is $23/24$ here against $23/25$ there, the same numerator over a
denominator that differs by the one excluded run.}
\label{tab:suite}
\renewcommand{\arraystretch}{1.0}
...
\midrule
banking   & $40.1\%$ & $97.7\%$ & $95.8\%$ \\
slack     & $60.9\%$ & $40.3\%$ & $11.5\%$ \\
...
\midrule
micro-average  & $0.84$ & $0.84$ & F1 $0.84$ \\
\multicolumn{4}{l}{single-primary exact-match: $45/54 = 83.3\%$} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
original      & $61.6$ & $31.7$ & $71.8$ \\
corrected     & $71.1$ & $42.3$ & $73.1$ \\
conservative  & $61.6$ & $31.7$ & $61.7$ \\
expanded      & $71.2$ & $45.5$ & $73.1$ \\
\bottomrule
\end{tabular}
\end{table}
...
$50\times$ replication and for an AgentDojo-derived log. Throughput stays near
$0.4$--$0.8$M events/s with flat $8$--$14$\,MB RSS, and parametric/timed formulas cost
about the same as generic ones. Replicating the STAC log $10\times$ yields exactly
$3470=10\times347$ firings, a soundness check on scale-up.
...
\midrule
STAC $\times1$          & $\approx0.4$M & $8$--$14$\,MB \\
...
IoT-lock policy, in four categories (compliant, violating, boundary, and
adversarially-ambiguous), roughly $100$ compliant and $100$ violating each. MonPoly
...
five generic obligations at $80.8\%$ detection / $46.5\%$ BFR to $79.8\%$ / $43.7\%$,
and the four-suite overall rates from the same generic union at $61.5\%$ / $31.5\%$ to
both-parametric $60.9\%$ / $29.8\%$. These pooled figures are unions over the full
...
at the \emph{action slot} --- the export slot BFR falls $9.9\%\!\to\!7.0\%$ and the
\texttt{send\_money} slot $44\%\!\to\!24\%$ --- which is where the provenance conjunct
applies.
