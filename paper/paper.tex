...
\end{tikzpicture}}
\looseness=-1 \caption{Offline pipeline. Recorded AgentDojo/STAC/R-Judge traces are canonicalised
...
provenance integrity). No LLM or agent runs.}
\label{fig:pipeline}
\end{figure}
...
and which MFOTL states declaratively instead of leaving to bespoke stateful code.
...
online, from inside the agent runtime, on three authored applications (HospitalGPT,
OptiGuide and a travel agent) driven by $20$ synthetic queries each. Our question is a
different one, and the difference is the corpus rather than the formalism: what is
measurable \emph{offline}, on the attack corpora the community already ships, and what
those corpora cannot support. That setting is not directly comparable, because those
applications ship no recorded trajectories to replay, and it is the reason we report on
$6{,}899$ AgentDojo runs, $483$ STAC chains, $571$ R-Judge records and $1{,}980$
$\tau$-bench trajectories rather than on authored queries. What formal RV adds in that setting is the combination:
trace-level scope, formal temporal semantics, data-parametric provenance and offline
evaluability in \emph{one} declarative substrate. The comparison is with what the cited systems were
shown to do, not with what they could in principle express.
AgentDojo~\cite{agentdojo2024}, AgentBench~\cite{agentbench2024} and
ToolEmu~\cite{toolemu2023} \emph{evaluate} agents, and the guardrails they benchmark are,
as reported, per-call or rule-based. AgentSpec~\cite{agentspec2026} has a formal rule
semantics and some history-aware rules ($\circ$), so a determined encoding could reach
further than Table~\ref{tab:matrix} shows; that table records evaluated configurations,
not impossibility.
...
MFOTL states and MonPoly checks conditions a stateless per-call predicate cannot. Four
...
flipped $94$--$99\%$ by one planted structured line, which binding provenance to the
issuing lookup closes completely on the tested attack family, at no cost in detection
or benign firing; a field ablation showing that the sound and unsound variants are
indistinguishable except under attack; and
...
and it asks two things of a future benchmark rather than one: the enforcement-ready trace
schema of \S\ref{sec:gap}, and an interaction protocol in which approval can occur at
all. The $\tau$-bench control shows the second is the binding constraint, since a
single-turn protocol cannot express approval however many fields its log carries. MFOTL/MonPoly is thus a reusable, auditable
