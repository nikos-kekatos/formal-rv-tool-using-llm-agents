...
\acmConference[CPSIoTSec '26]{Joint Workshop on CPS \& IoT Security and
  Privacy}{November 2026}{The Hague, Netherlands}
...
% Author list (same as the ARES/IWAPS paper). Non-anonymous; re-add the
% [anonymous] class option to blind the PDF for a double-blind review round.
...
--- with the real MonPoly engine (no LLM, no API key). RV flags $71.8\%$ of STAC
chains and detects $61.6\%$ of GPT-4o's \emph{successful} AgentDojo attacks at a
$31.7\%$ benign firing rate (BFR); we are explicit that here the generic obligations
reduce largely to \emph{typed-action detection}, since approvals are almost never
present. The formalism's distinctive value appears with \emph{parametric}
specifications that quantify over trace data: a payee-provenance obligation cuts
banking BFR from $44\%$ to $24\%$ and an external-recipient obligation cuts workspace
BFR from $15\%$ to $11\%$, both statistically significant (exact McNemar
$p<10^{-10}$). We further show an earliest-warning margin (obligations fire before
the final harmful call in $51.3\%$ of successful attacks), that the provenance
...
action. We position MFOTL/MonPoly as a reusable, auditable substrate for agent-trace
policies --- complementary to, not a replacement for, rule-based enforcement.
\end{abstract}
...
report it plainly: the same generic obligations fire on $31.7\%$ ($[24.1,40.4]$) of
\emph{benign} task runs, because a legitimate task also sends email, moves money and
changes config. Detection and BFR rise and fall together under \emph{generic}
specs.
...
attacks; BFR is on its benign (\textsf{attack}=none) runs. Per-obligation breakdown in
...
The parametric rows fire only when a relational/provenance condition over the trace
holds, so their verdict differs from the same call's typed-action verdict; the
generic detection numbers elsewhere are typed-action detection.}
\label{tab:param}
...
benchmarks lack (\S\ref{sec:gap}). Provenance integrity is a first-class requirement
for agent RV, not a canonicalisation detail.
...
flipped $94$--$99\%$ by one planted structured line, which we answer with a hardened
trusted-provenance specification; and a readiness scorecard showing current benchmarks
...
(and, on acceptance, an anonymous public repository), so the full pipeline is
...
on successful attacks, and RV BFR on benign runs.}
\label{tab:suite}
\renewcommand{\arraystretch}{1.0}
...
expected verdict on $780/780$ ($100\%$).

\paragraph{Combined parametric ablation} Applying \emph{both} parametric obligations
together, the banking$+$workspace pooled rates move from generic $80.8\%$ detection /
$46.5\%$ BFR to $79.8\%$ / $43.7\%$, and the four-suite overall rates from generic
$61.5\%$ / $31.5\%$ to both-parametric $60.9\%$ / $29.8\%$. Run-level BFR barely moves
