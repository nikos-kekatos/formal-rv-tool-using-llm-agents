...
AgentDojo attacks the generic obligations detect $70.2\%$ at a $32.5\%$ benign firing
...
\item Results on three corpora with Wilson $95\%$ intervals: $71.8\%$ of STAC chains
      flagged (MonPoly-confirmed); $70.2\%$ of GPT-4o's \emph{successful} AgentDojo
      attacks detected at a $32.5\%$ benign firing rate (BFR); the full R-Judge
...
small, and we report Wilson intervals on every rate (e.g.\ the $32.5\%$ aggregate BFR
...
$28.9\%$ ($1931/6680$). Our RV, applied to the identical runs, fires on $70.2\%$
...
report it plainly: the same generic obligations fire on $32.5\%$ ($40/123$, Wilson
$95\%$ $[24.9,41.2]$) of \emph{benign} task runs, because a legitimate task also sends
...
of \S\ref{sec:gap}. Second, \emph{a $32.5\%$ BFR is not a deployable operating point}.
...
\emph{overlap} --- one run can trip several obligations --- so they sum to $1699$, more
than the $1356$ runs detected; the union, not the sum, is the detection figure. Runs
...
RV detection on \emph{successful} attacks       & $1356/1931 = 70.2\%$ \\
RV fires on any attacked run                     & $2593/6680 = 38.8\%$ \\
RV BFR on benign runs                     & $40/123 = 32.5\%$ \\
\quad Wilson $95\%$ interval on BFR             & $[24.9,41.2]$ \\
\midrule
\multicolumn{2}{l}{\emph{by obligation, on the $1356$ detected (overlapping)}} \\
...
\quad sum of the five (overlap $>$ union)       & $1699$ \\
\bottomrule
\end{tabular}
\end{table}
...
final harmful call (\emph{preventive}) in $1117$ ($57.8\%$), fires only \emph{at} that
call (\emph{last-moment}) in $239$ ($12.4\%$), and never fires in $575$ ($29.8\%$);
where it is preventive the median detection lead is two tool-calls (mean $2.53$, max
...
obligations ($\mathrm{LR^+}=2.16$) to the parametric spec raises it on both suites ---
...
detection moves in $[70.2,80.0]\%$ and BFR in $[32.5,47.2]\%$, STAC detection stays in
...
enforcement) outside our scope. Generic specs have a high BFR ($32.5\%$), and the
...
detects $70.2\%$ of GPT-4o's successful AgentDojo attacks at $32.5\%$ BFR --- numbers
...
(preventive on $57.8\%$ of successful AgentDojo attacks, median lead two tool-calls);
...
=(1931-1356)/6680=8.6\%$; this is a descriptive count of successful attacks the
...
generic union fires on $23/25$ benign banking runs against $23/24$ here, the same
numerator over the one extra run; Table~\ref{tab:param}'s $11/25$ differs from both
because it is one obligation rather than the union.}
\label{tab:suite}
\renewcommand{\arraystretch}{1.0}
...
\midrule
original      & $70.2$ & $32.5$ & $71.8$ \\
corrected     & $79.9$ & $43.9$ & $73.1$ \\
conservative  & $70.2$ & $32.5$ & $61.7$ \\
expanded      & $80.0$ & $47.2$ & $73.1$ \\
\bottomrule
\end{tabular}
\end{table}
...
and the four-suite overall rates from the same generic union at $70.0\%$ / $32.3\%$ to
both-parametric $69.5\%$ / $30.6\%$. These pooled figures are unions over the full
